...
if(isset($_GET['id']))
{
	$sql1= "SELECT * FROM tab_color where id='$_GET[id]'";
$result = mysqli_query($con,$sql1);
while($row = mysqli_fetch_array($result))
  {
	$varid=$row['id']; 
    $varcl=$row['color_name'];
	}
}
if(isset($_POST['btnsub']))
	{
	 $sqlupd="update tab_color set color_name='$_POST[cltype]' where id='$_POST[txtid]'";
...
<input type="text" class="form-control" id="cltype" name="cltype"
 placeholder="Enter name of color" required="required" value="<?php echo $varcl; ?>">
</div>
</div>
